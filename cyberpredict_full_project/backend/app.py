...
# Jurisdictional Police & Regional Cluster Matrix
REGIONAL_MATRIX = {
    "east": {
...
# Initial seed database of active hotspots
active_hotspots_db = [
    {
        "id": "NCRP-2026-901",
...
def get_all_hotspots():
    """Retrieve all current cash-out threat hotspots prioritized by risk score."""
...
def report_fraud_and_auto_assign(payload: FraudComplaintRequest):
    """
    Ingest a 1930 / NCRP cyber fraud complaint, execute ML cash-out scoring, 
    and directly assign to the jurisdictional Cyber Police Station.
    """
    region = REGIONAL_MATRIX.get(payload.region_key)
    if not region:
        raise HTTPException(status_code=400, detail="Invalid region specified.")

    # 1. Feature Vectorization for ML inference
    features = np.array([[
        payload.amount_inr,
        payload.hour_of_day,
        payload.mule_hops,
    ...
    # 2. Predict Risk Score using trained Random Forest model
    ...
    # Add subtle jitter to exact coordinates for realistic pin distribution
